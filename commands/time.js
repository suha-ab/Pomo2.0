...
function createPomoFoundEmbed(interaction, time){
    return new EmbedBuilder()
    .setTitle(`${interaction.member.nickname}'s Pomo`)
    .setDescription(`${time} remaining. Keep going! :)`)
    .setColor('ec3946')
}
...
module.exports = {
    ...
    .setName('time')
    .setDescription('shows the remaining time in the current interval.'),
        ...
        if (myPomos.length == 0) await interaction.reply({embeds: [createNoSuchPomoEmbed(interaction)]})
            ...
            for(pomo of myPomos){
                if (pomo.interaction.user.username == interaction.user.username){
                    var time = pomo.remainingInTimer()
                    interaction.reply({embeds: [createPomoFoundEmbed(interaction, time)]})
                    found = true
                    break
                }
            }
